...
| 2.4 | ✅ **Log improvements** — a filter field, errors tinted red and counted, and it tails from an offset rather than re-reading the file every 1.5s | The viewer worked and did not help you read. All three controls this item named were built and then removed: follow is on by default and the tail is where you already are, wrap had no counterpart worth keeping once lines simply wrapped, and jump-to-first-error was a loupe glyph next to a filter field that does the same job better. Filtering for "error" shows every one of them, not just the first |
